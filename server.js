...
// HOME PAGE
app.get("/", (req, res) =>
  res.send("Karanks1436 Backend is Live 🚀 Nodemailer Working!")
);
...
// SEND LOGIN EMAIL (GET)
app.get("/send-login-email", async (req, res) => {
  const { name = "User", email } = req.query;
  ...
  try {
    const info = await sendWithRetries({
      ...
      html: `<h2>Hello ${name}</h2><p>You logged in at ${new Date().toLocaleString()}</p>`,
    });
...
// SEND CONTACT MESSAGE (GET)
app.get("/send-contact-message", async (req, res) => {
  const { name = "User", email, message = "" } = req.query;
  if (!email) return res.status(400).json({ error: "Email required" });
...
// BULK EMAIL (GET)
app.get("/send-bulk-email", async (req, res) => {
  const emails = req.query.emails?.split(",") || [];
  const message = req.query.message || "";
  const subject = req.query.subject || "🔔 Notification";
        ...
        try {
          const info = await sendWithRetries({
            ...
            html: `
              <h3>Notification</h3>
              <p>${message}</p>
              ${fileUrl ? `<a href="${fileUrl}" target="_blank">Attached File</a>` : ""}
            `,
          });
